  ...
  const DoctorCard = ({ doc }) => (
    <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 hover:shadow-xl transition duration-300 flex flex-col hover:border-[#800020]">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <p className="text-[#800020] font-medium text-[11px] uppercase tracking-wide mb-1 leading-tight">{doc.role}</p>
        <p className="text-gray-500 font-bold text-sm mb-2">{getText(doc.specialty)}</p>
        <p className="text-gray-500 text-xs mb-4 line-clamp-4 leading-relaxed">{getText(doc.description)}</p>
        ...
        <div className="mt-auto pt-4 border-t border-gray-100">
            ...
            {openBios[doc.id] && (
               <div className="text-xs text-gray-600 bg-gray-50 p-4 rounded mb-3 text-justify animate-in fade-in leading-relaxed border border-gray-100">
                 {getText(doc.biography)}
               </div>
            )}
            
            {doc.bookable && (
                <button onClick={() => setShowBookingModal(true)} className="w-full bg-[#800020] text-white py-2.5 rounded-lg text-sm font-bold hover:bg-[#600015] transition shadow-sm">
                    {t.doctors.book}
                </button>
            )}
        </div>
    ...
    <div className={`font-sans text-gray-700 relative h-full transition-colors duration-1000 ease-in-out ${bgColorClass}`}>
      ...
      <Modal isOpen={showAuthModal} onClose={() => setShowAuthModal(false)} title={authMode === 'login' ? "Connexion" : "Compte"}>
          <form onSubmit={handleAuth} className="space-y-4">
             ...
             <input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full p-3 border rounded focus:outline-none focus:border-[#800020]"/>
             <input type="password" placeholder="Mot de passe" value={password} onChange={e=>setPassword(e.target.value)} className="w-full p-3 border rounded focus:outline-none focus:border-[#800020]"/>
             <button type="submit" className="w-full bg-[#800020] text-white py-3 rounded font-bold hover:bg-[#600015] transition shadow">Entrer</button>
      ...
      {showDashboard && user ? (
        ...
        <div className="scroll-container" onScroll={(e) => setScrolled(e.target.scrollTop > 50)}>
           ...
           <section id="home" className="relative h-screen overflow-hidden bg-gray-900">
             ...
             {heroSlides.map((slide, index) => (
               <div key={index} className={`absolute inset-0 z-0 transition-opacity duration-1000 ease-in-out ${currentHeroSlide === index ? 'opacity-100' : 'opacity-0'}`}>
                 {slide.type === 'rubric' ? (
                   <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#800020] text-white p-6">
                     <div className="text-center w-full max-w-2xl mt-12 md:mt-0">
                       <div className="relative flex justify-center mb-8 group cursor-default">
                          <svg viewBox="0 0 400 200" className="w-full max-w-md h-auto drop-shadow-2xl group-hover:scale-105 transition duration-700">
                            <path d="M 50 100 L 150 100 L 165 60 L 195 150 L 215 80 L 230 100 L 350 100" fill="none" stroke="#d4af37" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity="0.6" />
                            <text x="175" y="115" textAnchor="end" fontSize="90" fontWeight="300" fontFamily="Georgia, serif" fill="#ffffff" letterSpacing="-4">E</text>
                            <text x="205" y="115" textAnchor="start" fontSize="90" fontWeight="300" fontFamily="Georgia, serif" fill="#ffffff" letterSpacing="-4">N</text>
                            <text x="200" y="165" textAnchor="middle" fontSize="18" fontWeight="400" fontFamily="Arial, sans-serif" fill="#ffffff" letterSpacing="6">EVA NIYIBIZI</text>
                            <text x="200" y="185" textAnchor="middle" fontSize="10" fontWeight="300" fontFamily="Arial, sans-serif" fill="#d4af37" letterSpacing="3">INTERNISTE & URGENTISTE</text>
                          </svg>
                       </div>
                       <p className="text-xl md:text-2xl font-serif max-w-lg mx-auto leading-relaxed drop-shadow-md mt-4">{t.carousel.rubricSubtitle}</p>
                       <button onClick={() => setShowBookingModal(true)} className="mt-8 bg-white text-[#800020] px-8 py-3 rounded-full font-bold hover:bg-gray-100 transition shadow-xl">{t.hero.cta}</button>
                     </div>
                   </div>
                 ) : (
                   <div className="absolute inset-0 z-0 bg-cover bg-center" style={{backgroundImage: `url("${slide.image}")`}}>
                     ...
                     <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-black/30 flex flex-col items-center justify-center text-white p-6 text-center">
                        <div className="max-w-2xl mt-12 md:mt-0">
                           <h2 className="text-4xl md:text-6xl font-bold mb-4 font-serif drop-shadow-lg">{slide.title}</h2>
                           <p className="text-xl md:text-2xl mx-auto font-light leading-relaxed drop-shadow-md mb-8">{slide.subtitle}</p>
                           <button onClick={() => setShowBookingModal(true)} className="bg-[#800020] text-white px-8 py-3 rounded-full font-bold hover:bg-[#600018] transition shadow-xl">{t.hero.cta}</button>
                        </div>
                     </div>
                   </div>
                 )}
             ...
             <button onClick={prevHeroSlide} className="absolute left-6 top-1/2 z-10 p-2 rounded-full bg-white/10 text-white hover:bg-white/30 transition">
               <ChevronLeft size={32} />
             </button>
             <button onClick={nextHeroSlide} className="absolute right-6 top-1/2 z-10 p-2 rounded-full bg-white/10 text-white hover:bg-white/30 transition">
                   ...
                   <div className="text-center mb-16">
                       ...
                       <div className="flex flex-wrap justify-center gap-6 mt-8">
                           {CLINIC_VALUES.map((val, idx) => (
                               <div key={idx} className="flex items-center gap-2 bg-gray-50 px-5 py-3 rounded-full border border-gray-100 shadow-sm text-gray-700 font-medium">
                                   <IconMapper type={val.icon} className="text-[#800020] w-5 h-5"/> {getText(val.title)}
                               </div>
                           ))}
                       </div>
                       <div className="mt-8 max-w-2xl mx-auto text-gray-600 italic bg-gray-50 p-4 rounded-lg border border-gray-200">
                   ...
                   <div className="grid md:grid-cols-2 gap-12 items-center bg-gray-50 rounded-2xl p-8 md:p-12 shadow-sm border border-gray-100">
                       <div>
                           ...
                           <div className="mt-6 flex flex-wrap gap-2">
                               <span className="bg-white border border-gray-200 px-3 py-1 rounded text-sm font-bold text-gray-600 shadow-sm">UNIGE</span>
                               <span className="bg-white border border-gray-200 px-3 py-1 rounded text-sm font-bold text-gray-600 shadow-sm">2ème Bachelor</span>
                               <span className="bg-white border border-gray-200 px-3 py-1 rounded text-sm font-bold text-gray-600 shadow-sm">3ème Master</span>
                           </div>
                       </div>
                       <div>
           ...
           <section id="clinic" className="py-20 bg-white">
               <div className="container mx-auto px-6">
                   <h2 className="text-3xl font-bold text-center text-[#800020] mb-16">{t.sections.services}</h2>
                   ...
                   <div className="grid md:grid-cols-2 gap-12">
                       <div className="space-y-8">
                           <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                               <h3 className="font-bold text-xl text-[#800020] border-b border-gray-200 pb-3 mb-6 flex items-center gap-2">
                               ...
                               <ul className="grid grid-cols-2 gap-3">
                                   {TECHNICAL_PLATFORM.careRoom.map((cr, i) => (
                                       <li key={i} className="flex items-center gap-2 text-gray-700 text-sm">
                                         <div className="w-1.5 h-1.5 rounded-full bg-[#800020]"></div> {getText(cr)}
                                       </li>
                                   ))}
                               </ul>
                           </div>
           ...
           <section className="py-16 bg-[#800020] text-white mt-12">
              <div className="container mx-auto px-6 text-center">
                <h2 className="text-3xl md:text-4xl font-bold mb-6">Besoin d'un rendez-vous ?</h2>
                <button 
                  onClick={() => setShowBookingModal(true)} 
                  className="bg-white text-[#800020] px-10 py-4 rounded-full font-bold text-xl hover:bg-gray-100 transition shadow-lg transform hover:scale-105"
                >
                  Prendre Rendez-vous
                </button>
              </div>
              ...
              <div className="container mx-auto px-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                   <div>
                     ...
                     <p className="text-[#800020] bg-white px-3 py-1 rounded w-fit text-sm font-bold mt-2">niyibizi@hin.ch</p>
                   </div>
                   ...
                   <div>
                       ...
                       <p className="text-gray-500 text-sm mt-8">© 2026 Cabinet du Seujet.</p>
                   </div>
